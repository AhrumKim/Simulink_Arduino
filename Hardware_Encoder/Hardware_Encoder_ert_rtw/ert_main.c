 ...
 * C/C++ source code generated on : Tue Oct 15 09:46:16 2024
 *
...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) Hardware_Encoder_M->Timing.clockTick0;
  Hardware_Encoder_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Hardware_Encoder_M, 0);
  ...
  errorCode = extmodeInit(Hardware_Encoder_M->extModeInfo, &rtmGetTFinal
    (Hardware_Encoder_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Hardware_Encoder_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Hardware_Encoder_M);
