...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) Final_Project_1_M->Timing.clockTick0;
  Final_Project_1_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Final_Project_1_M, 0);
  ...
  Final_Project_1_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(Final_Project_1_M->extModeInfo, &rtmGetTFinal
    (Final_Project_1_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Final_Project_1_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Final_Project_1_M);
