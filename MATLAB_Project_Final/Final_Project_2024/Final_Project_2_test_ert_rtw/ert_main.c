...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Final_Project_2_test_M, 0);
  ...
  Final_Project_2_test_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(Final_Project_2_test_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Final_Project_2_test_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Final_Project_2_test_M);
  ...
  /* Terminate model */
  Final_Project_2_test_terminate();
