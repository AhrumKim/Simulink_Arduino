...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    HC_SR04_ultrasonic_M->Timing.clockTick0;
  HC_SR04_ultrasonic_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(HC_SR04_ultrasonic_M, 0);
  ...
  HC_SR04_ultrasonic_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(HC_SR04_ultrasonic_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(HC_SR04_ultrasonic_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(HC_SR04_ultrasonic_M);
