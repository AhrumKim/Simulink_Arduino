...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(if0x2Celse_M, 0);
  ...
  if0x2Celse_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(if0x2Celse_M->extModeInfo, &rtmGetTFinal(if0x2Celse_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(if0x2Celse_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(if0x2Celse_M);
